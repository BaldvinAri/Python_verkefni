...
def print_sorted(dict_countries, dict_players):
    ...
    sorted_countries = sorted(dict_countries.items())
    for key,players in sorted_countries:
        print(f"{key} ({len(dict_countries[key])}) ({get_average_rating(players, dict_players):.1f}):")
